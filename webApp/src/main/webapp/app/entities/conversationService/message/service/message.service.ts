...
type RestOf<T extends IMessage | NewMessage> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class MessageService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/messages', 'conversationservice');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/messages/_search', 'conversationservice');
  ...
  protected convertDateFromServer(restMessage: RestMessage): IMessage {
    return {
      ...restMessage,
      date: restMessage.date ? dayjs(restMessage.date) : undefined,
    };
  }
