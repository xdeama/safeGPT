...
export class ConversationComponent implements OnInit {
  ...
  protected fillComponentAttributesFromResponseBody(data: IConversation[] | null): IConversation[] {
    ...
    if ('prev' in this.links) {
      const conversationsNew = this.conversations ?? [];
      if (data) {
        for (const d of data) {
          if (conversationsNew.map(op => op.id).indexOf(d.id) === -1) {
            conversationsNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.conversationService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
