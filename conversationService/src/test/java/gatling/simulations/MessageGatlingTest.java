        ...
        .on(
            exec(
                http("Get all messages")
                    .get("/services/conversationservice/api/messages")
                ...
                .exec(
                    http("Create new message")
                        .post("/services/conversationservice/api/messages")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created message")
                            .get("/services/conversationservice${new_message_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created message")
                        .delete("/services/conversationservice${new_message_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
