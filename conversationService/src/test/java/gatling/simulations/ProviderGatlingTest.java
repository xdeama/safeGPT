 ...
 */
public class ProviderGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all providers")
                    .get("/services/conversationservice/api/providers")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new provider")
                        .post("/services/conversationservice/api/providers")
                        .headers(headers_http_authenticated)
                ...
                .on(
                    exec(
                        http("Get created provider")
                            .get("/services/conversationservice${new_provider_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created provider")
                        .delete("/services/conversationservice${new_provider_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Provider entity").exec(scn);
