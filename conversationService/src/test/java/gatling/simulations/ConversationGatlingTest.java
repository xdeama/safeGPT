...
public class ConversationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all conversations")
                    .get("/services/conversationservice/api/conversations")
                ...
                .exec(
                    http("Create new conversation")
                        .post("/services/conversationservice/api/conversations")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"startDate\": \"2020-01-01T00:00:00.000Z\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .on(
                    exec(
                        http("Get created conversation")
                            .get("/services/conversationservice${new_conversation_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created conversation")
                        .delete("/services/conversationservice${new_conversation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
